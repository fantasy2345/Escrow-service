...
// redux
import { getCustomerList } from '../redux/slices/user';
// routes
...
export default function customerList() {
  ...
  const { customerList } = useSelector((state) => state.user);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = customerList.map((n) => n.id);
