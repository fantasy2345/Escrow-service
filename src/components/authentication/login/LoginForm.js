      ...
      try {
        try{
          ...
          if(error.response.data.message == "Please verify your email!") {
            await axios.post(
              `${process.env.REACT_APP_API_URL}/setup/send/verify/code`, {
                email: values.email,
              }
            );
            
            history.push(`/auth/verify/${values.email}`);
          } else if (error.response.data.message == "Incorrect account") {
            enqueueSnackbar('The account is not registered.', {
              variant: 'error',
              action: (key) => (
                <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                  <Icon icon={closeFill} />
