...
export default function ChatRoomOneParticipant({
  ...
  const role = ["Admin", "Editor", "Author", "Client"];
  const displayRole = role[participants.length != 0? participant.position: 5];
    ...
    <Box {...other}>
      ...
      >
        <Avatar
          ...
          src={`${process.env.REACT_APP_API_URL}/${participant.avatar}`}
