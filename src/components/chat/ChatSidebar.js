...
export default function ChatSidebar() {
  ...
  const handleChangeSearch = async (event) => {
    try {
      ...
      if (value) {
        ...
        const results = contacts.allIds.filter((id) => (
        contacts.byId[id].name.toLowerCase().includes(value)
        ));

        let searchResult = [];
        results.map((result) => {
          searchResult.push(contacts.byId[result])
        })
  ...
  const handleSearchSelect = (name) => {
    setSearchFocused(false);
    setSearchQuery('');
    history.push(`${PATH_DASHBOARD.chat.root}/${name}`);
  };
